...
def information_retrieval_linear(sents, is_roberta=False):
    device = torch.device("cuda:0")
    id2label = config.id2label
    if is_roberta:
        model_dir = r'C:\Users\Super-IdoI\Desktop\dataset-ecir\Fintech-Key-Phrase\BERT_Softmax\experiments\financial\roberta_softmax'  # './experiments/financial/roberta_softmax/'
    else:
        model_dir = r'C:\Users\Super-IdoI\Desktop\dataset-ecir\Fintech-Key-Phrase\BERT_Softmax\experiments\financial\bert_softmax'  # './experiments/financial/bert_softmax/'
    ...
    with torch.no_grad():
        ...
        pred_tags = [[id2label.get(idx) for idx in indices] for indices in np.argmax(batch_output, axis=2)]
    results = []
...
if __name__ == '__main__':
    ...
    res = information_retrieval_linear(texts, False)
