...
def information_retrieval_lstm_crf(sents, is_roberta=False):
    device = torch.device("cuda:0")
    id2label = config.id2label
    if is_roberta:
        model_dir = r'C:\Users\Super-IdoI\Desktop\dataset-ecir\Fintech-Key-Phrase\BERT_LSTM_CRF\experiments\financial\roberta_lstm_crf'  # './experiments/financial/roberta_lstm_crf/'
    else:
        model_dir = r'C:\Users\Super-IdoI\Desktop\dataset-ecir\Fintech-Key-Phrase\BERT_LSTM_CRF\experiments\financial\bert_lstm_crf'  # './experiments/financial/bert_lstm_crf/'
    ...
    if SAVE:
        ...
        torch.save(model, os.path.join(model_dir, 'pytorch_model.pth'))
        return
    else:
        model = torch.load(model_dir+'\\pytorch_model.pth')
...
if __name__ == '__main__':
    ...
    res = information_retrieval_lstm_crf(texts, False)
